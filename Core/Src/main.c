...
#include "mavlink/common/mavlink.h"/*Yolun dogru oldugundan emin ol*/
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
      // --- A. BAŞLANGIÇTA SENSÖRÜ UYANDIR (WHILE DÖNGÜSÜNDEN ÖNCE -- BİR KERE ÇALIŞIR) ---
      HAL_Delay(100);
      // 2. WhoAmI Kontrolü (Opsiyonel ama iyi)
      ...
      // 3. Sensör Ayarlarını Gönder (TEK TEK YAZMA - EN GÜVENLİSİ)
    ...
    while (1)
    {
      // LED Yak Söndür (Döngü çalışıyor mu?)
      HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12); // Yeşil LED Toggle
      // VERİLERİ OKU (TEK TEK - GARANTİ YÖNTEM)
      uint8_t xl, xh, yl, yh;
